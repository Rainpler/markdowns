...
拿我们刚刚的例子来说，我们定义了一个``ThreadLocal<Integer>``类型的ThreadLocal，是不是还能再定义一个ThreadLocal<Boolean>类型的？那么对于同一个线程来说，他就会拥有了两个ThreadLocal副本，那么这个数组，就是用于存放线程所拥有的ThreadLocal对象的。
...
```
为什么每个线程都输出5？难道他们没有独自保存自己的Number 副本吗？为什么其他线程还是能够修改这个值？仔细考察ThreadLocal 和Thead 的代码，因为``number``对象被定义为了static，所以ThreadLocalMap 中保存的其实是对象的一个引用，这样的话，当有其他线程对这个引用指向的对象实例做修改时，其实也同时影响了所有的线程持有的对象引用所指向的同一个对象实例。
