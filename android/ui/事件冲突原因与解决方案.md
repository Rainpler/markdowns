...
```
当onInterceptTouchEvent方法返回为true的时候，表示拦截子view的touch事件，这样会导致viewpager可以作为滑动，但是listview无法滑动。如果返回为false的话，表示不拦截子view，这时候会导致listview可以滑动，但viewpager无法滑动，因此此时该事件。如果我们直接将该方法注释掉，这时viewpager和listview都可以滑动了。这就是一个事件冲突的例子，在这里，Google为事件冲突进行了处理。z
...
这里再看一个SwiperRefreshLayout和ViewPager的事件冲突，在这里Google同样为我们进行了解决，如果我们要自己处理的话，跟上面有略微的不同。如果是从SwiperRefreshLayout角度去进行外部处理的话，跟上面的解决方法是一样的。但是如果我们尝试在ViewPager中参照上面的方法进行内部处理的话，会发现效果没有实现。
```java
@Override
    public boolean dispathTouchEvent(MotionEvent event) {
        ...
...
```
经过排查，可以发现，在ACTION_DOWN事件之后调用了``getParent().requestDisallowInterceptTouchEvent(true)``，但是ACTION_MOVE事件仍然走了swiperrefreshlayout的onInterceptTouchEvent方法，说明该方法并没有生效。我们来看一下swiperrefreshLayout中的requestDisallowInterceptTouchEvent实现。
```java
...
```
在调用super.requestDisallowInterceptTouchEvent(b)之前，它会先进行一个条件判断。而对于我们的viewpager来说，它并不是嵌套滑动的控件，因此不满足ViewCompat.isNestedScrollingEnabled(this.mTarget)，导致条件判断不满足。

所以我们需要主动调用viewpager的setNestedScrollingEnabled方法。
```java
case ACTION_DOWN:
